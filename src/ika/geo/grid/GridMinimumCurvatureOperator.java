 ...
 * @author jenny
 */
public class GridMinimumCurvatureOperator extends ThreadedGridOperator {
    ...
    public void operate(GeoGrid src, GeoGrid dst, int startRow, int endRow) {
        ...
        if (startRow == 0) {
            for (int col = 0; col < cols; col++) {
                this.operateBorder(src, dst, col, 0, cellSize);
            }
        }
        ...
        if (endRow == rows - 1) {
            for (int col = 0; col < cols; col++) {
                this.operateBorder(src, dst, col, rows - 1, cellSize);
            }
        }

        // left column
        for (int row = firstInteriorRow; row < lastInteriorRow; row++) {
            this.operateBorder(src, dst, 0, row, cellSize);
        }

        // right column
        for (int row = firstInteriorRow; row < lastInteriorRow; row++) {
            this.operateBorder(src, dst, cols - 1, row, cellSize);
        }
